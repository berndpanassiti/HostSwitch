...
#' @param n_sim simulation number plotted. The "n_sim" values possible are saved in the HostSwitch object, the plot from the first simulation is shown as default (n_sim = 1).An error message "Error in FUN(X[[i]], ...) : subscript out of bounds" will be returned, if the called simulation is not present in the object HostSwitch.
#'
#' @details The function plots the simulation for the dispersal, colonization (successful host switch), and reproduction of a consumer. The X-axis shows the number of generations, the y-axis the phenotype values of the consumer.
#'
