...
#' @usage data(parli)
#'
#' @format An object of class "list". The list contains three matrices: $Cephaloleia, $Cacopsylla, and $SarsMers.
#'\enumerate{
#' \item{\emph{parli$Cephaloleia} is a data frame with 11 rows (parameters) and 8 columns (variables).}
#'
#' The parameters are classified in three groups:
#'\itemize{
#'   \item{Arbitrary varying: mig (probability of migration) and jump_back (to the original Resource).}
#'   \item{Arbitrary fixed: K (carrying capacity), N-generations, seed, n_sim, pRes_min, pRes_max, sd (standard deviation of mutation), and sigma (standard deviation of survival).}
#'   \item{From real data: b (net reproduction rate) (Garcia-Robledo and Horvitz, 2011).}
#'}
#'  The column name acronyms are assigned after:
#'\itemize{
#'   \item{two \emph{Cephaloleia} species: \bold{Cb} for \emph{C. belti} and \bold{Cp} for \emph{C. placida}.}
#'   \item{two levels of variation for mig: \bold{mL} for low and \bold{mH} for high migration.}
#'   \item{two levels for jump_back: \bold{jY} for jumping back and \bold{jN} for no jumping back individuals.}
#'}
#'
...
#'   \item{From real data: b, sigma and mig (Malagnini et al. 2010, 2011; Mayer et al. 2013).}
#'}
#' The column name acronyms are assigned after:
#'\itemize{
#'   \item{two \emph{Cacopsylla melanoneura} populations: \bold{CmA} for \emph{C. melanoneura} adapted to Apple and \bold{CmH} for \emph{C. melanoneura} adapted to hawthorn.}
#'   \item{two levels of variation for sd: \bold{sdL} for low and \bold{sdH} for high standard deviation of mutation.}
#'   \item{two levels for jump_back: \bold{jY} for jumping back and \bold{jN} for no jumping back individuals.}
#'}
#'
...
#' The column name acronyms are assigned after:
#'\itemize{
#'   \item{two coronaviruses: \bold{Sars} for \emph{Sarbecovirus} sp. and \bold{Mers} for \emph{Merbacovirus} sp.}
#'   \item{three levels of variation for mig: \bold{migL} for low, \bold{migM} for medium and \bold{migH} for high probability of migration.}
#'}
#' }
#'
#' @keywords datasets
#' @source Garcia-Robledo C and Horvitz CC (2011) Experimental demography and the vital rates of generalist and specialist insect herbivores on native and novel host plants. Journal of Animal Ecology, 80(5):976-989.
#'
