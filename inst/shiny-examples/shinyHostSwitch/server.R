...
server <- function(input, output, session) {
  ...
  HostSwitch_simulated_quantities <- reactive({
    ...
    isolate(HostSwitch::simHostSwitch(K=input$K,b=input$b, mig=input$mig, sd=input$sd, sigma=input$sigma, pRes_min=1, pRes_max=10, n_generation=input$n_generation,iter=1))
  ...
  output$HostSwitchSummary = renderTable({
    data.frame("Number of parasite jumps"= length(which(HostSwitch_simulated_quantities()$pInd_jump_sim[[1]]>0)),
